...
resolve_name() {
    ...
    #   slack/send
    #
    if [[ ${#prefixes[@]} -eq 0 && ${#parts[@]} -ge 3 ]]; then
    # Skip first catalog category:
    #
    # communication/discord/send
    #        ^ skip
    #
...
elif [[ ${#prefixes[@]} -eq 0 && ${#parts[@]} -eq 2 ]]; then
    # Example:
    # storage/query
    # becomes:
    # storage-query
    ...
    # lib.rs always belongs to its parent folder.
# Example:
# catalog/communication/email/lib.rs
# -> email_lib
#
...
# If parent folder is empty, fallback to explicit prefix
if [[ -z "$lib_name" && ${#prefixes[@]} -gt 0 ]]; then
    lib_name="${prefixes[-1]}"
fi
        
    echo "$(to_snake_case "$lib_name")_lib"
